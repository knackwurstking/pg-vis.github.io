...
`;let Ce=class{constructor(t){this.root=t}get gap(){return this.root.getAttribute("gap")||Tt}set gap(t){t===null?this.root.removeAttribute("gap"):this.root.setAttribute("gap",t)}};const j=class j extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=xe,this.cleanup=new c,this.ui=new Ce(this),this.updateStyle()}connectedCallback(){}disconnectedCallback(){this.cleanup.run()}attributeChangedCallback(t,e,i){switch(t){case"gap":this.updateStyle({gap:i||Tt});break}}updateStyle({gap:t=Tt}={}){this.shadowRoot.querySelector("style").textContent=`
        ...
        `}};a(j,"register",()=>{customElements.get("ui-flex-grid")||customElements.define("ui-flex-grid",j)}),a(j,"observedAttributes",["gap"]);let K=j;const Le=l`
<style>
...
`,ut=class ut extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=ze}};a(ut,"register",()=>{customElements.get("svg-search")||customElements.define("svg-search",ut)});let jt=ut;const je=l`
<style>
    :host {
        width: 100%;
        height: 100%;
...
`,ct=class ct extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=je}};a(ct,"register",()=>{customElements.get("svg-menu")||customElements.define("svg-menu",ct)});let Bt=ct;const Be=l`
<style>
    :host {
        width: 100%;
        height: 100%;
...
`,ht=class ht extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=Be}};a(ht,"register",()=>{customElements.get("svg-share")||customElements.define("svg-share",ht)});let Ft=ht;const Fe=l`
<style>
    * { box-sizing: border-box; }
...
`)}}is(t,e){return!!`${t.from}..${t.to} ${t.alert} ${t.desc.join(`
`)}`.match(e)}async onClick(t){const e=t.currentTarget,i=this.getAlertFromLI(e);console.warn(i)}};a(yt,"register",()=>{Y.register(),Zt.register(),customElements.define("alert-lists-page",yt)});let U=yt;const ii=l`
