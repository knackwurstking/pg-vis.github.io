...
`,Et=class Et extends m{constructor(){super(),this.innerHTML=ai,this.ui={...this.ui,events:new v},this.data=null}connectedCallback(){super.connectedCallback();const t=this.querySelector("ui-button"),e=this.querySelector("ui-icon-button");this.cleanup.add(t.ui.events.on("click",()=>{this.ui.events.dispatch("click",this.data)})),this.cleanup.add(e.ui.events.on("click",()=>{this.ui.events.dispatch("delete",this.data)}))}set(t){this.data=t,this.setText(t.title,t.user||null)}setText(t,e=null){const i=document.createElement("span");i.innerHTML=a` <ui-primary>${t}</ui-primary> `,e!==null&&(i.innerHTML+=a`<ui-secondary>${e}</ui-secondary>`),this.querySelector("ui-button").appendChild(i)}};n(Et,"register",()=>{M.register(),m.register(),A.register(),k.register(),b.register(),S.register(),T.register(),x.register(),customElements.define("pg-drawer-item-alert-list",Et)});let U=Et;const li=a`
