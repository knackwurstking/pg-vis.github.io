...
`;let $e=class{constructor(t){this.root=t}get gap(){return this.root.getAttribute("gap")||qt}set gap(t){t===null?this.root.removeAttribute("gap"):this.root.setAttribute("gap",t)}};const z=class z extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=Me,this.cleanup=new m,this.ui=new $e(this),this.updateStyle()}connectedCallback(){}disconnectedCallback(){this.cleanup.run()}attributeChangedCallback(t,e,i){switch(t){case"gap":this.updateStyle({gap:i||qt});break}}updateStyle({gap:t=qt}={}){this.shadowRoot.querySelector("style").textContent=`
        ...
        `}};a(z,"register",()=>{customElements.get("ui-flex-grid-row")||customElements.define("ui-flex-grid-row",z)}),a(z,"observedAttributes",["gap"]);let W=z;const Nt="0",Te=c`
<style></style>
<slot></slot>
`;let He=class{constructor(t){this.root=t}get gap(){return this.root.getAttribute("gap")||Nt}set gap(t){t===null?this.root.removeAttribute("gap"):this.root.setAttribute("gap",t)}};const j=class j extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=Te,this.cleanup=new m,this.ui=new He(this),this.updateStyle()}connectedCallback(){}disconnectedCallback(){this.cleanup.run()}attributeChangedCallback(t,e,i){switch(t){case"gap":this.updateStyle({gap:i||Nt});break}}updateStyle({gap:t=Nt}={}){this.shadowRoot.querySelector("style").textContent=`
            :host {
                display: flex !important;
...
`;var $,ne;let qe=(ne=class{constructor(t){f(this,$,void 0);b(this,$,t)}getLeftSlot(){return[...n(this,$).querySelectorAll('[slot="left"]')]}getCenterSlot(){return[...n(this,$).querySelectorAll('[slot="center"]')]}getRightSlot(){return[...n(this,$).querySelectorAll('[slot="right"]')]}},$=new WeakMap,ne);const nt=class nt extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=Re,this.cleanup=new m,this.ui=new qe(this)}connectedCallback(){}disconnectedCallback(){this.cleanup.run()}};a(nt,"register",()=>{W.register(),customElements.get("ui-app-bar")||customElements.define("ui-app-bar",nt)});let Dt=nt;const Ne=c`
<style>
    * { box-sizing: border-box; }
...
`,Mt=class Mt extends M{constructor(){super(),this.innerHTML=gi,this.ui.name="vis-data"}};a(Mt,"register",()=>{customElements.define("vis-data-page",Mt)});let ot=Mt;function fi(o){const t=o.map(i=>`${i.title}:${i.user}`).sort(),e=[];for(const i of t)e.push(o.find(s=>`${s.title}:${s.user}`===i));return e}const bi="v0.0.1",vi=c`
    <ui-store
